...
const App: React.FC = () => {
  ...
  // 初始化API密钥
  useEffect(() => {
    const savedApiKey = localStorage.getItem('doubao-api-key');
    if (savedApiKey) {
  ...
  }, [isInitialState, saveCustomStyle, addToHistory, apiKey]);
  ...
  const handleFuseStyles = useCallback(async () => {
    ...
    // 设置风格融合的进度步骤
    const fuseProgress: ProgressStep[] = [
      { name: '分析选中的风格特征...', status: 'in-progress' },
      { name: '调用AI进行风格融合...', status: 'pending' },
      { name: '生成新的融合风格描述...', status: 'pending' },
      { name: '保存新风格到本地...', status: 'pending' },
    ];
    setProgress(fuseProgress);

    const updateProgress = (stepIndex: number, status: ProgressStatus) => {
      setProgress(prev => {
        if (!prev) return null;
        const newProgress = [...prev];
  ...
  }, [fuseSelection, newStyleName, allStyles, saveCustomStyle, customStyles, apiKey]);
