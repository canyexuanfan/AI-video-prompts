...
const callArkAPI = async (messages: any[], systemInstruction: string, apiKey: string): Promise<string> => {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      throw new Error('API密钥无效或已过期，请检查您的火山引擎豆包API密钥是否正确');
    } else if (response.status === 403) {
      throw new Error('API密钥权限不足，请确认您的API密钥具有访问豆包模型的权限');
...
export const generateDescription = async (
    ...
    try {
        ...
        console.error("Ark API Service Error:", error);
